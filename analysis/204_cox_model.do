...
foreach fail in $ae_combined $ae_disease $ae_disease_serious  {
	stset stop_`fail', id(patient_id) origin(time start_date) enter(time start_date) failure(fail_`fail'==1) 		
	stptime 
...
** Hazard of events						
tempname cox_model_summary_sens2
postfile `cox_model_summary_sens2' str20(model) str20(failure) ///
	hr_sot lc_sot uc_sot hr_pax lc_pax uc_pax hr_mol lc_mol uc_mol ///
	using "$projectdir/output/tables/cox_model_summary_sens2", replace							 
foreach fail in $ae_combined {
	stset stop_`fail', id(patient_id) origin(time start_date) enter(time start_date) failure(fail_`fail'==1) 
	...
	bys time_`fail': stcox i.drug $`model', vce(robust) 
		...
		post `cox_model_summary_sens2' ("`model'") ("`fail'") (`hr_sot') (`lc_sot') (`uc_sot') (`hr_pax') (`lc_pax') (`uc_pax') (`hr_mol') (`lc_mol') (`uc_mol')				
}
}
postclose `cox_model_summary_sens2'

tempname cox_model_disease_sens2
postfile `cox_model_disease_sens2' str20(model) str20(failure) ///
	hr_sot lc_sot uc_sot hr_pax lc_pax uc_pax hr_mol lc_mol uc_mol ///
	using "$projectdir/output/tables/cox_model_disease_sens2", replace							 
foreach fail in $ae_diseaseae_ $ae_disease_serious {
	stset stop_`fail', id(patient_id) origin(time start_date) enter(time start_date) failure(fail_`fail'==1) 
	foreach model in crude agesex adj adj2 adj3 adj4{
		stcox $`model', vce(robust)
					...
					local lc_mol = b[5,4]
					local uc_mol = b[6,4]			
		post `cox_model_disease_sens2' ("`model'") ("`fail'") (`hr_sot') (`lc_sot') (`uc_sot') (`hr_pax') (`lc_pax') (`uc_pax') (`hr_mol') (`lc_mol') (`uc_mol')				
}
}
postclose `cox_model_disease_sens2'
...
*******************************************************************************************************************
** Sensitivity analysis 1 Time varying covariate **
...
** Hazard of events						
tempname cox_model_summary_sens
postfile `cox_model_summary_sens' str20(model) str20(failure) ///
	hr_sot lc_sot uc_sot hr_pax lc_pax uc_pax hr_mol lc_mol uc_mol ///
	using "$projectdir/output/tables/cox_model_summary_sens", replace							 
foreach fail in $ae_combined {
	stset stop_`fail', id(patient_id) origin(time start_date) enter(time start_date) failure(fail_`fail'==1) 
	foreach model in crude agesex adj adj2 adj3 adj4{
		stcox $`model', vce(robust)
		...
		post `cox_model_summary_sens' ("`model'") ("`fail'") (`hr_sot') (`lc_sot') (`uc_sot') (`hr_pax') (`lc_pax') (`uc_pax') (`hr_mol') (`lc_mol') (`uc_mol')				
}
}
postclose `cox_model_summary_sens'

tempname cox_model_disease_sens
postfile `cox_model_disease_sens' str20(model) str20(failure) ///
	hr_sot lc_sot uc_sot hr_pax lc_pax uc_pax hr_mol lc_mol uc_mol ///
	using "$projectdir/output/tables/cox_model_disease_sens", replace							 
foreach fail in $ae_diseaseae_ $ae_disease_serious {
	stset stop_`fail', id(patient_id) origin(time start_date) enter(time start_date) failure(fail_`fail'==1) 
	foreach model in crude agesex adj adj2 adj3 adj4{
		stcox $`model', vce(robust)
					...
					local lc_mol = b[5,4]
					local uc_mol = b[6,4]			
		post `cox_model_disease_sens' ("`model'") ("`fail'") (`hr_sot') (`lc_sot') (`uc_sot') (`hr_pax') (`lc_pax') (`uc_pax') (`hr_mol') (`lc_mol') (`uc_mol')				
}
}
postclose `cox_model_disease_sens'


*******************************************************************************************************************
** Sensitivity analysis 2 **
use "$projectdir/output/data/sensitivity_analysis", clear
keep if covid_test==1
