...
* Open a log file
cap log close
log using "$logdir/cleaning_dataset_newstart.log", replace

*Set Ado file path
adopath + "$projectdir/analysis/ado"

* 	Import control dataset
import delimited "$projectdir/output/input_combined.csv", clear
...
*******************************************************************
count if start_date==.
count if start_date!=covid_test_positive_date 
...
*** combined serious ae from hosp and A&E
...
global ae_spc_gp			ae_diverticulitis_snomed		///
							...
							ae_dizziness_ae					///
							ae_headache_ae										
global ae_spc_hosp			ae_diverticulitis_icd_prim		///
							ae_taste_icd_prim				///
							ae_rash_icd_prim				///
							ae_diarrhoea_icd_prim			///
							ae_diarrhoeal_icd_prim			///
...
global ae_drug_gp	 		ae_anaphylaxis_snomed			///	
							...
							ae_severedrug_ae				///
							ae_nonsevere_drug_ae				
global ae_drug_hosp			ae_anaphylaxis_icd_prim			///
							ae_severedrug_icd_prim			///	
							ae_severedrug_sjs_icd_prim			
global ae_imae_gp			new_ae_ra_snomed 				///
							...
							new_ae_axspa_ae					///
							new_ae_ibd_ae								
global ae_imae_hosp			new_ae_ra_icd_prim 				///
							new_ae_sle_icd_prim				///
...
global ae_disease_serious	ae_diverticulitis_serious 		///
							...
							ae_axspa_serious				///
							ae_ibd_serious 						
egen ae_spc_all = rmin($ae_spc_gp $ae_spc_hosp)
egen ae_spc_serious = rmin($ae_spc_hosp)
egen ae_drug_all = rmin($ae_drug_gp $ae_drug_hosp)
egen ae_drug_serious = rmin($ae_drug_hosp)
egen ae_imae_all = rmin($ae_imae_gp $ae_imae_hosp)
egen ae_imae_serious = rmin($ae_imae_hosp)
egen ae_all = rmin($ae_disease)
egen ae_all_serious = rmin($ae_disease_serious)
...
********************************************************
*	A. INCLUSION ONLY WITH POSITIVE COVID TEST	*
********************************************************
** inclusion criteria*
...
drop if start_date!=covid_test_positive_date
...
gen pre_drug_test=0 if dataset==1 & ((date_treated-covid_test_positive_date>=0)&(date_treated-covid_test_positive_date<=3))
replace pre_drug_test=1 if dataset==1 & ((date_treated-covid_test_positive_date>=4)&(date_treated-covid_test_positive_date<=5))
replace pre_drug_test=2 if dataset==1 & ((date_treated-covid_test_positive_date>=6)&(date_treated-covid_test_positive_date<=7))
replace pre_drug_test=3 if dataset==1 & ((date_treated-covid_test_positive_date>7)&(date_treated-covid_test_positive_date<21))
replace pre_drug_test=4 if dataset==1 & ((date_treated-covid_test_positive_date<0)|(date_treated-covid_test_positive_date>=21))
replace pre_drug_test=5 if dataset==1 & covid_test_positive_date==.
label define pre_drug_test 0 "<=3 days" 1 "4-5 days"  2 "6-7 days" 3 ">7 days & <21 days" 4 "treatment >21 day or <0" 5 "no test", replace 
...
** covid test within 5 days of treatment
gen pre_drug_test_time = date_treated-covid_test_positive_date
sum pre_drug_test_time, det
gen pre_drug_test=0 if drug>0 & ((date_treated-covid_test_positive_date>=0)&(date_treated-covid_test_positive_date<=3))
replace pre_drug_test=1 if drug>0 & ((date_treated-covid_test_positive_date==4)&(date_treated-covid_test_positive_date==5))
replace pre_drug_test=2 if drug>0 & ((date_treated-covid_test_positive_date==6)&(date_treated-covid_test_positive_date==7))
replace pre_drug_test=3 if drug>0 & ((date_treated-covid_test_positive_date>7)&(date_treated-covid_test_positive_date<21))
replace pre_drug_test=4 if drug>0 & ((date_treated-covid_test_positive_date<0)|(date_treated-covid_test_positive_date>=21))
label define pre_drug_test 0 "<=3 days" 1 "4-5 days"  2 "6-7 days" 3 ">7 days & <21 days" 4 "treatment >21 day or <0" , replace 
label values pre_drug_test pre_drug_test
tab pre_drug_test drug,m
drop if drug>0 & pre_drug_test>=2
sum pre_drug_test_time, det
...
foreach x in $ae_spc_gp $ae_spc_hosp $ae_drug_gp $ae_drug_hosp $ae_imae_gp $ae_imae_hosp {
				display "`x'"
...
save "$projectdir/output/data/main_primary_analysis.dta", replace
...
***********************************************************************
*	B. INCLUSION OF TREATMENT GROUP WITHOUT POSITIVE COVID TEST	*
***********************************************************************
** inclusion criteria*
keep if age>=18 & age<110
keep if sex==0 | sex==1
...
** keep all with and without positive covid test
...
** time between covid and treatment
gen pre_drug_test_time = date_treated-covid_test_positive_date
sum pre_drug_test_time, det
gen pre_drug_test=0 if drug>0 & ((date_treated-covid_test_positive_date>=0)&(date_treated-covid_test_positive_date<=3))
replace pre_drug_test=1 if drug>0 & ((date_treated-covid_test_positive_date==4)&(date_treated-covid_test_positive_date==5))
...
replace pre_drug_test=3 if drug>0 & ((date_treated-covid_test_positive_date>7)&(date_treated-covid_test_positive_date<21))
replace pre_drug_test=4 if drug>0 & ((date_treated-covid_test_positive_date<0)|(date_treated-covid_test_positive_date>=21))
replace pre_drug_test=5 if drug>0 & covid_test_positive_date==. 
...
save "$projectdir/output/data/main_sensivity_analysis.dta", replace
