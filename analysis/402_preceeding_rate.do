...
foreach i of local levels {
	...
	if all_events!=. {
		...
		replace all_uci_round =  0 if all_events_round==0
}
	...
	if control_events!=. {
		...
		replace control_uci_round =  0 if control_events_round==0
}
	if sot_events!=. {
		...
		replace sot_uci_round =  0 if sot_events_round==0
}
	...
	if pax_events_round!=. {
		...
		replace pax_uci_round =  0 if pax_events_round==0
}
	...
	if mol_events_round!=. {
		...
		replace mol_uci_round =  0 if mol_rate_round==0
}
