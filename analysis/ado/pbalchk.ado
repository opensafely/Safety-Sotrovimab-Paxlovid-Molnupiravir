...
*! Version: 4.0.0
*! Author:  Mark Lunt
*! Date:    November 16, 2018 @ 10:31:07
...
version 11
   ...
   if "$track_pbalchk" != "0" {
      ...
      capture trackuse, progname("pbalchk") av("4.0.0") aid("NA") options("`tuoptions'") package("pbalchk")
      if _rc != 0 {
   ...
   if "`xline'" != "" {
	...
	if "`cvars'" ~= "" {
		...
		unab cvars : _C*
      foreach var of varlist `croots' {
          local vlab : value label `var' 
         if "`vlab'" != "" {
            foreach cvar of varlist _C`var'* {
               local lab : variable label `cvar'
               if regexm("`lab'", "(.*==)([0-9 ]+)$") {
                  local val = regexs(2)
                  local pref = regexs(1)
                  local thislab : label `vlab' `val'
                  local lab `pref'`thislab'
                  label variable `cvar' "`lab'"
               }
            }
         }
      }
			...
			foreach root in `croots' {
               ...
               local var_col = colnumb(`meandiff', "`var'")
					if regexm("`var'", "_C`root'_") {
						...
						capture logit `var' `treat' `istrata' `pwtexp' if `touse', iter(100)
						if _rc == 0 {
                     tempname e v
                     matrix `e' = e(b)
                     matrix `v' = e(V)
                     ...
                     if colnumb("`e'", "`treat'") < . {
                        local z = el("`e'", 1, 1) /sqrt(el("`v'", 1, 1))
                        local fstat = (`z')^2
                        local p_value = 2*(1-normal(abs(`z')))
                     }
                     else {
				...
				if "`f'" == "f" | "`p'" == "p" {
					mlogit `root' `treat' `istrata' `pwtexp' if `touse', iter(100)
					testparm `treat'
					local fstat = `r(chi2)'
					local p_value = `r(p)'
   ...
   if "`graph'" != "" {
		quietly {
         ...
         egen rank = rank(abs(unadj)), unique
         //      noi list
         foreach num of numlist 1/`size' {
